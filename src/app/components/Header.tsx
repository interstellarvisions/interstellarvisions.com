...
const aiMediaItems = [
  { label: "WHAT WE CREATE", id: "what-we-create" },
...
export default function Header({ scrolled }: HeaderProps) {
  ...
  useEffect(() => {
    const sectionIds = ["hero", "what-we-create", "our-work", "about", "faq", "contact"];
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (intersecting) {
          ...
          if (id === "what-we-create" || id === "our-work") {
  ...
  const NavButton = ({ label, id }: { label: string; id: string }) => {
    ...
    return (
      <button
        ...
        className={`relative text-sm tracking-wider transition-colors duration-300 ${isActive ? "text-cyan-400" : "hover:text-cyan-400"}`}
      >
        {label}
        {isActive && (
          isSafari
  ...
  const DropdownMenu = () => (
    <div
      className="rounded-xl overflow-hidden shadow-xl shadow-black/40"
      style={{ background: "linear-gradient(135deg, rgba(6,182,212,0.15), rgba(0,0,0,0.95))", border: "1px solid rgba(6,182,212,0.25)" }}
    >
      {aiMediaItems.map((item, i) => (
        <button
          key={item.id}
          onClick={() => scrollToSection(item.id)}
          className={`w-full text-left px-5 py-3 text-sm tracking-wider transition-all duration-200 flex items-center gap-2 group hover:text-cyan-400 hover:bg-white/5 ${i < aiMediaItems.length - 1 ? "border-b border-white/5" : ""}`}
        >
  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-black/90 border-b border-violet-500/10 shadow-lg shadow-black/20" : "bg-transparent"}`}
    >
      <div className="max-w-[1400px] mx-auto px-6 py-2 flex items-center justify-between">
        <button onClick={() => scrollToSection("hero")} className="flex items-center -my-2 cursor-pointer group">
          <img
            ...
            className="h-20 w-auto scale-120 origin-left transition-all duration-300 group-hover:opacity-80 group-hover:drop-shadow-[0_0_18px_rgba(124,58,237,0.55)]"
          />
        </button>
              ...
              <AnimatePresence>
                {aiMediaOpen && (
                  <motion.div
                    ...
                    className="absolute top-full left-1/2 -translate-x-1/2 w-52 pt-3"
                  >
                    <DropdownMenu />
                  </motion.div>
                )}
        ...
        {/* Mobile toggle */}
        <button
          className="md:hidden w-10 h-10 rounded-full border border-white/10 flex items-center justify-center hover:border-cyan-500/40 hover:bg-cyan-500/10 transition-all duration-300"
          ...
          <nav className="md:hidden" style={{ background: "rgba(0,0,0,0.95)", borderTop: "1px solid rgba(6,182,212,0.15)" }}>
            <div className="flex flex-col p-6 gap-1">
              ...
              {navItems.slice(1).map((item) => (
                <button key={item.id} onClick={() => scrollToSection(item.id)} className={`text-left text-sm tracking-wider py-3 px-3 rounded-lg transition-all duration-200 ${activeSection === item.id ? "text-cyan-400 bg-cyan-500/10" : "hover:text-cyan-400 hover:bg-white/5"}`}>
                  {item.label}
                </button>
              ))}
            </div>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            ...
            >
              <div className="flex flex-col p-6 gap-1">
                {[
                  <button key="home" onClick={() => scrollToSection("hero")} className={`text-left text-sm tracking-wider py-3 px-3 rounded-lg transition-all duration-200 ${activeSection === "hero" ? "text-cyan-400 bg-cyan-500/10" : "hover:text-cyan-400 hover:bg-white/5"}`}>HOME</button>,
                  <div key="ai-media">
                    <button onClick={() => setMobileAiMediaOpen(!mobileAiMediaOpen)} className={`flex items-center gap-2 text-left text-sm tracking-wider py-3 px-3 rounded-lg w-full transition-all duration-200 ${isAiMediaActive ? "text-cyan-400 bg-cyan-500/10" : "hover:text-cyan-400 hover:bg-white/5"}`}>
                      AI MEDIA <ChevronDown size={14} className={`transition-transform duration-300 ${mobileAiMediaOpen ? "rotate-180" : ""}`} />
                    </button>
                  ...
                  <motion.div key={i} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07, duration: 0.25, ease: "easeOut" }}>
                    {child}
                  </motion.div>
                ))}
              </div>
